...
describe('hidden projects, persisted', () => {
  it('defaults to nothing hidden', () => {
    ...
    expect(isProjectHidden(EMPTY_PREFS, 'factory', 'p1')).toBe(false);
  });

  it('survives a write/read round trip', () => {
    const storage = store();
    writePrefs(storage, setProjectHidden(EMPTY_PREFS, 'factory', 'p1', true));
    expect(isProjectHidden(readPrefs(storage), 'factory', 'p1')).toBe(true);
  });

  it('hides only the source it was told about', () => {
    const next = setProjectHidden(EMPTY_PREFS, 'factory', 'p1', true);
    expect(isProjectHidden(next, 'claude-code', 'p1')).toBe(false);
  });

  it('restores a hidden project, leaving no residue behind', () => {
    const hidden = setProjectHidden(EMPTY_PREFS, 'factory', 'p1', true);
    const back = setProjectHidden(hidden, 'factory', 'p1', false);
    expect(isProjectHidden(back, 'factory', 'p1')).toBe(false);
