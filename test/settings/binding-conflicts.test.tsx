 ...
 *   2. bind `close` to the now-free `r`
 *   3. reset `rename`
 *
 * Step 3 deleted the override with no check at all, so `rename` returned to
 * its default `r` — which `close` now held. `buildTables` gives the override
 * precedence, so `r` invoked `close` while the editor and the sheet went on
...
/** After steps 1 and 2: `rename` on `b`, `close` on the freed `r`. */
const AFTER_TWO_STEPS: Prefs = {
  ...EMPTY_PREFS,
  keyBindings: { rename: ['b'], close: ['r'] },
};
...
describe('the reset that could steal a key', () => {
  it('refuses it, and names the action that owns the key now', () => {
    ...
    // Refused: nothing was written, so `r` still reaches `close` and `rename`
    // still answers the key the operator moved it to.
    expect(onChange).not.toHaveBeenCalled();
    expect(message()).toContain('"r"');
    expect(message()).toContain(labelOf('close', AFTER_TWO_STEPS));
  });
  ...
  it('performs a reset that takes nobody’s key', () => {
    ...
    const prefs: Prefs = { ...EMPTY_PREFS, keyBindings: { rename: ['b'], close: ['q'] } };
    const { onChange } = open(prefs);
    fireEvent.click(reset('rename'));
    expect(onChange).toHaveBeenCalledTimes(1);
    expect(changed(onChange).keyBindings).toEqual({ close: ['q'] });
    expect(message()).toBe('');
  });
...
describe('a dead binding is shown, not hidden', () => {
  it('says on the row that its key reaches something else', () => {
    ...
    expect(dead.getAttribute('aria-label')).toContain(labelOf('close', CONTESTED));
  });

  it('names the contested key once for the whole section', () => {
    open(CONTESTED);
    expect(clashNote()).toContain('"r"');
    expect(clashNote()).toContain(labelOf('close', CONTESTED));
